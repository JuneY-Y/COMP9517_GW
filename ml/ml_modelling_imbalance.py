...
datasets_dir = Path("datasets_longtail")
...
class GMMClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        # Initialize GMM with balanced component assignment
        init_means = []

        # For each class, pick representative samples for initialization
        components_per_class = max(1, self.n_components // n_classes)
        ...
        for c in classes:
            ...
            # If few samples, use them all
            if len(X_class) <= components_per_class:
                init_means.extend(X_class)
            else:
                # Use k-means to find representative samples
                from sklearn.cluster import KMeans
                kmeans = KMeans(n_clusters=components_per_class, random_state=42)
                kmeans.fit(X_class)
                init_means.extend(kmeans.cluster_centers_)

        # If we have exactly n_components means, use them for initialization
        if len(init_means) == self.n_components:
            self.gmm = GaussianMixture(
...
class BalancedXGBClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, n_estimators=100, max_depth=3, learning_rate=0.1, subsample=0.8,
                 colsample_bytree=0.8, objective="multi:softprob", random_state=42,
                 use_label_encoder=False, eval_metric='mlogloss', n_jobs=-1, **kwargs):
...
def create_stacking_classifier(estimators=None, final_estimator=None, cv=5):
    if estimators is None:
        ...
        estimators = [
            ('rf', BalancedRandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')),
            ('svm', SVC(probability=True, random_state=42, class_weight='balanced')),
            ('log', LogisticRegression(C=1.0, penalty='l1', solver='saga', random_state=42, class_weight='balanced')),
            ('xgb', BalancedXGBClassifier(use_label_encoder=False, eval_metric='mlogloss', random_state=42))
        ]
...
def try_hyperparameters(X_train, y_train, X_val, y_val, model_type, class_names, pca_n_components):
    ...
    if model_type == 'SVM':
        ...
        model_params = [
            {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.1, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 200, 'max_depth': 3, 'learning_rate': 0.1, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 100, 'max_depth': 5, 'learning_rate': 0.1, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.05, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.1, 'subsample': 1.0, 'colsample_bytree': 1.0, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.1, 'subsample': 0.6, 'colsample_bytree': 0.6, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 100, 'max_depth': 7, 'learning_rate': 0.05, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15},
            {'n_estimators': 300, 'max_depth': 5, 'learning_rate': 0.01, 'subsample': 0.8, 'colsample_bytree': 0.8, 'objective': 'multi:softprob', 'num_class': 15}
        ]
    elif model_type == 'CatBoost':
        model_params = [
            {'iterations': 100, 'learning_rate': 0.1, 'depth': 6, 'l2_leaf_reg': 3, 'loss_function': 'MultiClass',
    'auto_class_weights': 'Balanced'},
            {'iterations': 200, 'learning_rate': 0.1, 'depth': 6, 'l2_leaf_reg': 3, 'loss_function': 'MultiClass',
    'auto_class_weights': 'Balanced'},
            {'iterations': 100, 'learning_rate': 0.05, 'depth': 6, 'l2_leaf_reg': 3, 'loss_function': 'MultiClass',
    'auto_class_weights': 'Balanced'},
            {'iterations': 100, 'learning_rate': 0.1, 'depth': 6, 'l2_leaf_reg': 1, 'loss_function': 'MultiClass',
    'auto_class_weights': 'Balanced'},
            {'iterations': 100, 'learning_rate': 0.1, 'depth': 6, 'l2_leaf_reg': 10, 'loss_function': 'MultiClass',
    'auto_class_weights': 'Balanced'}
        ]
        ...
        model_params = [
            ...
            {'C': 10.0, 'penalty': 'l2', 'solver': 'newton-cg', 'class_weight': 'balanced'},
            {'C': 1.0, 'penalty': 'l1', 'solver': 'saga', 'class_weight': 'balanced'},
            {'C': 10.0, 'penalty': 'l1', 'solver': 'saga', 'class_weight': 'balanced'},
            {'C': 1.0, 'penalty': 'elasticnet', 'solver': 'saga', 'l1_ratio': 0.5, 'class_weight': 'balanced'}
        ]
        ...
        model_params = [
            {'estimators': base_estimators, 'final_estimator': LogisticRegression(random_state=42, class_weight='balanced'), 'cv': 3},
            {'estimators': base_estimators, 'final_estimator': LogisticRegression(random_state=42, class_weight='balanced'), 'cv': 5},
            {'estimators': base_estimators, 'final_estimator': BalancedRandomForestClassifier(n_estimators=50, random_state=42, class_weight='balanced'), 'cv': 3},
    ...
    for n_components in pca_components:
        for params in model_params:
            ...
            # Add the model
            if model_type == 'SVM':
                pipeline_steps.append(('model', SVC(probability=True, random_state=42, class_weight='balanced', **params)))
            elif model_type == 'KNN':
                pipeline_steps.append(('model', KNeighborsClassifier(**params)))
            elif model_type == 'K-means':
                pipeline_steps.append(('model', KMeansClassifier(**params)))
            elif model_type == 'RF':
                pipeline_steps.append(('model', BalancedRandomForestClassifier(random_state=42, class_weight='balanced', **params)))
            elif model_type == 'XGBoost':
                pipeline_steps.append(('model', BalancedXGBClassifier(
                    random_state=42,
                    **params
                )))
            elif model_type == 'CatBoost':
...
def train_final_model(X_combined, y_combined, model_type, best_params, pca_n_components):
    ...
    # Add the model
    if model_type == 'SVM':
        pipeline_steps.append(('model', SVC(probability=True, random_state=42, class_weight='balanced', **model_params)))
    elif model_type == 'KNN':
        pipeline_steps.append(('model', KNeighborsClassifier(**model_params)))
    elif model_type == 'K-means':
        pipeline_steps.append(('model', KMeansClassifier(**model_params)))
    elif model_type == 'RF':
        pipeline_steps.append(('model', BalancedRandomForestClassifier(random_state=42, class_weight='balanced', **model_params)))
    elif model_type == 'XGBoost':
        pipeline_steps.append(('model', BalancedXGBClassifier(
            random_state=42,
            **model_params
...
def main(sample_size=None, feature_type='combined', n_augmentations=5, batch_size=32):
    ...
    for model_type, model in best_models.items():
        # Only include standard scikit-learn classifiers that have predict_proba
        if model_type in ['SVM', 'KNN', 'RF', 'LogisticRegression'] and hasattr(model, 'predict_proba'):
            voting_classifiers.append((model_type, model))
        # Handle xgboost separately
        if model_type == 'XGBoost' and hasattr(model, 'predict_proba'):
            # Get the actual XGBoost model from custom wrapper
            if hasattr(model, 'steps') and model.steps[-1][0] == 'model':
                # If it's in a pipeline, extract it
                xgb_model = model.steps[-1][1].model  # Get the actual XGBoost model
                voting_classifiers.append((model_type, xgb_model))
            else:
                print(f"Skipping {model_type} for voting ensemble")
...
if __name__ == "__main__":
    ...
    if quick_test:
        ...
        total_sample_size = 10  # Total samples per class across all splits
