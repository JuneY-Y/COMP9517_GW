...
train_dataset = datasets.ImageFolder(root='longtail/longtail/train', transform=train_transforms)
val_dataset   = datasets.ImageFolder(root='longtail/longtail/test', transform=val_transforms)
...
model2 = timm.create_model(model_name, pretrained=False, num_classes=num_classes).to(device)
...
counter = 0
start_time = time.time()
save_path = 'longtail_model_2.pth'

for epoch in range(20):
    model2.train()
    for imgs, labels in tail_loader:
    ...
    with torch.no_grad():
        for imgs, labels in val_loader:
            imgs, labels = imgs.to(device), labels.to(device)
            preds = model2(imgs).argmax(dim=1)
            correct += (preds == labels).sum().item()
            total   += labels.size(0)
    val_acc = correct / total
    print(f"[Stage2] Epoch {epoch+1} | Val Acc: {val_acc:.4f}")
...
print(f"Stage 2 done in {(time.time()-start_time):.1f}s")